import {Component} from 'react'
import Cookies from 'js-cookie'
import {Redirect, Link} from 'react-router-dom'
import Loader from 'react-loader-spinner'
import {formatDistanceToNow} from 'date-fns'
import {BsDot, BsSearch} from 'react-icons/bs'
import {RiCloseLine} from 'react-icons/ri'
import Header from '../Header'
import NavBar from '../NavBar'

import './index.css'

const apiStatusConstants = {
  initial: 'INITIAL',
  success: 'SUCCESS',
  failure: 'FAILURE',
  inProgress: 'IN_PROGRESS',
}

const Card = props => {
  const {cardDetails} = props
  const {thumbnailUrl, title, channel, publishedAt, viewCount, id} = cardDetails
  const {name, profileImageUrl} = channel
  const years = formatDistanceToNow(new Date(publishedAt))
  const inputString = years
  const lastTwoParts = inputString.split(' ').slice(-2).join(' ')

  return (
    <li>
      <Link
        to={`/videos/${id}`}
        style={{textDecoration: 'none', color: '#000'}}
      >
        <img src={thumbnailUrl} alt="video thumbnail" className="video-t-img" />
        <div style={{display: 'flex'}}>
          <img
            src={profileImageUrl}
            alt="channel logo"
            className="profile-t-img"
          />
          <div>
            <p>{title}</p>
            <p>{name}</p>
            <div style={{display: 'flex', alignItems: 'center'}}>
              <p>{viewCount} views </p>
              <BsDot style={{fontSize: '1.5rem'}} />
              <p>{lastTwoParts}</p>
            </div>
          </div>
        </div>
      </Link>
    </li>
  )
}

class Home extends Component {
  state = {
    videos: [],
    apiStatus: apiStatusConstants.initial,
    searchIn: '',
    banner: true,
  }

  componentDidMount = () => {
    this.apiFetchHome()
  }

  apiFetchHome = async () => {
    this.setState({
      apiStatus: apiStatusConstants.inProgress,
    })
    const {searchIn} = this.state
    const jwtToken = Cookies.get('jwt_token')
    const url = `https://apis.ccbp.in/videos/all?search=${searchIn}`
    const options = {
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
      method: 'GET',
    }
    const response = await fetch(url, options)
    if (response.ok === true) {
      const fetchedData = await response.json()
      const updatedData = fetchedData.videos.map(each => ({
        channel: {
          name: each.channel.name,
          profileImageUrl: each.channel.profile_image_url,
        },
        publishedAt: each.published_at,
        thumbnailUrl: each.thumbnail_url,
        id: each.id,
        viewCount: each.view_count,
        title: each.title,
      }))
      this.setState({
        videos: updatedData,
        apiStatus: apiStatusConstants.success,
      })
    }
    if (response.ok === false) {
      this.setState({
        apiStatus: apiStatusConstants.failure,
      })
    }
  }

  status = () => {
    const {apiStatus} = this.state
    switch (apiStatus) {
      case apiStatusConstants.success:
        return this.renderSuccessView()
      case apiStatusConstants.failure:
        return this.renderFailureView()
      case apiStatusConstants.inProgress:
        return this.renderLoader()
      default:
        return null
    }
  }

  renderSuccessView = () => {
    const {videos} = this.state
    return (
      <ul className="video-card-con">
        {videos.length !== 0 &&
          videos.map(each => <Card key={each.id} cardDetails={each} />)}
        {videos.length === 0 && (
          <div className="failure-view-con">
            <img
              src="https://assets.ccbp.in/frontend/react-js/nxt-watch-no-search-results-img.png"
              alt="no videos"
              className="no-video-image"
            />
            <h1>No Search results found</h1>
            <p>Try different key words or remove search filter</p>
            <div>
              <button
                type="button"
                onClick={this.apiFetchHome}
                className="login-button"
              >
                Retry
              </button>
            </div>
          </div>
        )}
      </ul>
    )
  }

  renderFailureView = () => (
    <div className="failure-view-con">
      <img
        src="https://assets.ccbp.in/frontend/react-js/nxt-watch-failure-view-light-theme-img.png"
        alt="failure view"
        className="failure-view-image"
      />
      <h1>Oops! Something Went Wrong</h1>
      <p>
        We are having some trouble to complete your request.
        <br />
        Please try again
      </p>
      <div>
        <button
          type="button"
          onClick={this.apiFetchHome}
          className="login-button"
        >
          Retry
        </button>
      </div>
    </div>
  )

  renderLoader = () => (
    <div className="loader-container" data-testid="loader">
      <Loader type="ThreeDots" color="#3b82f6" height="50" width="50" />
    </div>
  )

  searchOnChange = event => {
    this.setState({searchIn: event.target.value})
  }

  searchBtn = () => {
    this.apiFetchHome()
  }

  toggleBanner = () => {
    this.setState({banner: false})
  }

  render() {
    const jwtToken = Cookies.get('jwt_token')
    if (jwtToken === undefined) {
      return <Redirect to="/login" />
    }
    const {banner} = this.state

    return (
      <div style={{minHeight: '100vh'}}>
        <Header />
        <div style={{display: 'flex', minHeight: '88vh'}}>
          <NavBar />
          <div style={{margin: '0', backgroundColor: '#f1f1f1', width: '100%'}}>
            <div
              className={`banner-con ${!banner ? 'banner-close' : ''}`}
              data-testid="banner"
            >
              <div>
                <img
                  src="https://assets.ccbp.in/frontend/react-js/nxt-watch-logo-light-theme-img.png"
                  className="website-logo-image"
                  alt="nxt watch logo"
                />
                <p>Buy Nxt Watch Premium</p>
                <button type="button" className="get-it">
                  GET IT NOW
                </button>
              </div>
              <button
                type="button"
                data-testid="close"
                style={{
                  backgroundColor: 'transparent',
                  border: '0',
                  cursor: 'pointer',
                }}
                onClick={this.toggleBanner}
              >
                <RiCloseLine style={{fontSize: '1.5rem'}} />
              </button>
            </div>
            <div className="input-con">
              <input
                type="search"
                onChange={this.searchOnChange}
                className="search-input"
              />
              <button
                type="button"
                onClick={this.searchBtn}
                className="s-btn"
                data-testid="searchButton"
              >
                <BsSearch className="search-icon" />
              </button>
            </div>
            {this.status()}
          </div>
        </div>
      </div>
    )
  }
}
export default Home
